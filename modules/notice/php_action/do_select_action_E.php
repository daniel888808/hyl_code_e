<?php
    ...
    class do_select_action implements action_listener{
        public function actionPerformed(event_message $em) {
            ...
            $sql = "SELECT * FROM `household_user` A JOIN `public_facilities` B ON A.household_profile_id = B.id JOIN `household_profile` C ON A.household_profile_id = C.id";
            ...
            if($result){
                ...
                if($result){
                    ...
                    $sql = "SELECT * FROM `construction_project`";
                    $stmt = $conn->prepare($sql);
                    $result = $stmt->execute();
                    if($result){
